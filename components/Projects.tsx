        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-8">
          {projects.map((project, index) => (
            <motion.div
              ...
              className="group bg-white rounded-lg md:rounded-2xl overflow-hidden border border-slate-100 shadow-lg hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 flex flex-col h-full"
            >
              <div className="relative h-24 sm:h-48 overflow-hidden flex-shrink-0">
              ...
              <div className="p-2 md:p-6 flex flex-col flex-grow">
                <div className="flex gap-1 md:gap-2 mb-1.5 md:mb-4 flex-wrap content-start">
                  {project.tags.slice(0, 4).map(tag => (
                    <span key={tag} className="text-[7px] sm:text-xs font-mono px-1 py-0.5 md:px-2 md:py-1 rounded bg-slate-100 text-slate-600 border border-slate-200 leading-none">
                      {tag}
                    </span>
                  ))}
                  {project.tags.length > 4 && (
                     <span className="text-[7px] sm:text-xs font-mono px-1 py-0.5 md:px-2 md:py-1 rounded bg-slate-50 text-slate-500 border border-slate-100 leading-none">+{project.tags.length - 4}</span>
                  )}
                </div>
                
                <h3 className="text-xs sm:text-xl font-bold text-slate-900 mb-1 md:mb-2 group-hover:text-primary transition-colors leading-tight line-clamp-1">{project.title}</h3>
                <p className="text-slate-600 text-[9px] sm:text-sm mb-2 md:mb-6 flex-grow leading-tight md:leading-relaxed line-clamp-3 md:line-clamp-none">
                  {project.description}
                </p>
                
                <div className="flex gap-2 md:gap-4 mt-auto pt-2 md:pt-4 border-t border-slate-50 flex-wrap">
                  {project.demoUrl && project.demoUrl !== "#" && (
                    <a href={project.demoUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 md:gap-2 text-[9px] md:text-sm font-medium text-slate-800 hover:text-primary transition-colors bg-slate-50 px-1.5 py-0.5 rounded md:bg-transparent md:p-0">
                      {project.demoUrl.includes("linkedin.com") ? (
                        <>
                          <PlayCircle className="w-3 h-3 md:w-4 md:h-4" /> <span className="md:hidden">Demo</span><span className="hidden md:inline">Watch Demo</span>
                        </>
                      ) : (
                        <>
                          <ExternalLink className="w-3 h-3 md:w-4 md:h-4" /> <span className="md:hidden">Live</span><span className="hidden md:inline">Live Demo</span>
                        </>
                      )}
                    </a>
                  )}
                  {project.githubUrl && project.githubUrl !== "#" && (
                    <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 md:gap-2 text-[9px] md:text-sm font-medium text-slate-800 hover:text-primary transition-colors bg-slate-50 px-1.5 py-0.5 rounded md:bg-transparent md:p-0">
                      <Github className="w-3 h-3 md:w-4 md:h-4" /> Code
                    </a>
                  )}
                  {!project.githubUrl && !project.demoUrl && (
                     <span className="text-[9px] md:text-sm text-slate-400 italic">Coming soon</span>
                  )}
                </div>
              </div>
            </motion.div>
